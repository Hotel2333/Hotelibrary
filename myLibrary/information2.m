...
@interface information2 ()<UITableViewDelegate,UITableViewDataSource,NSFetchedResultsControllerDelegate>{
...
- (void)layoutHeaderView{
    ...
    [edit setBackgroundImage:[UIImage imageNamed:@"借阅.png"] forState:UIControlStateNormal];
    [headerTwo addSubview:edit];
    [edit addTarget:self action:@selector(goBorrow) forControlEvents:UIControlEventTouchUpInside];
...
- (void)goBorrow{
    ...
    UIAlertAction * next = [UIAlertAction actionWithTitle:@"是的" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        NSPredicate * pre = [NSPredicate predicateWithFormat:@"book_id LIKE %@",_bookID];
        _request.predicate = pre;
        NSArray * tmp = [_moc executeFetchRequest:_request error:nil];
        Book * obj = tmp[0];
