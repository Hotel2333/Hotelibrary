...
@interface record ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)goInit{
    ...
    NSLog(@"共找到%d条记录",(int)_array.count);
    ...
    if(_array.count!=0){
        ...
        if([obj isKindOfClass:[Borrow class]]){
            ...
            if(arr.count!=0){
                ...
                NSLog(@"[b]第%d行的书名为%@",(int)indexPath.row,book.book_name);
            }else{
                cell.textLabel.text = @"错误:该图书已删除";
            }
            //根据id找到读者名字
            _request = [NSFetchRequest fetchRequestWithEntityName:@"User"];
            if(b.reader_id == nil){
                cell.detailTextLabel.text = @"Not Found";
                NSLog(@"[b]第%d行的读者找不到",(int)indexPath.row);
            }else{
                ...
                if(arr.count!=0){
                    ...
                    NSLog(@"[b]第%d行的读者是%@",(int)indexPath.row,user.name);
                }else{
                    cell.detailTextLabel.text = @"Not Found";
                    NSLog(@"[b]第%d行的读者找不到",(int)indexPath.row);
                }
            ...
            if(arr.count!=0){
                Book * book = arr[0];
                cell.textLabel.text = book.book_name;
                NSLog(@"[r]第%d行的书名为%@",(int)indexPath.row,book.book_name);
            }else{
                cell.textLabel.text = @"错误:该图书已删除";
            }
            ...
            if(arr.count!=0){
                User * user = arr[0];
                cell.detailTextLabel.text = user.name;
                NSLog(@"[r]第%d行的读者是%@",(int)indexPath.row,user.name);
            }else{
                cell.detailTextLabel.text = @"Not Found";
                NSLog(@"[r]第%d行的读者找不到",(int)indexPath.row);
            }
            ...
            if(arr.count!=0){
                Book * book = arr[0];
                cell.textLabel.text = book.book_name;
                NSLog(@"[p]第%d行的书名为%@",(int)indexPath.row,book.book_name);
            }else{
                cell.textLabel.text = @"错误:该图书已删除";
            }
            ...
            if(arr.count!=0){
                User * user = arr[0];
                cell.detailTextLabel.text = user.name;
                NSLog(@"[p]第%d行的读者是%@",(int)indexPath.row,user.name);
            }else{
                cell.detailTextLabel.text = @"Not Found";
                NSLog(@"[p]第%d行的读者找不到",(int)indexPath.row);
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    NSObject * object = _array[indexPath.row];
    if([object isKindOfClass:[Borrow class]]){
        ...
        /*NSDateFormatter * formatter = [[NSDateFormatter alloc]init];
        formatter.dateFormat = @"yyyy年MM月dd日HH时mm分";
        NSDate * borrDate = [formatter dateFromString:obj.borr_date];
        NSDate * retuDate = [borrDate getPriousorLaterDateFromDate:3];*/
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"应还时间:%@",obj.borr_date];
        }];
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"管理员:%@",obj.manager_id];
        }];
        UIAlertAction * returnBook = [UIAlertAction actionWithTitle:@"删除" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [alertView addAction:returnBook];
        [alertView addAction:cancel];
        [self presentViewController:alertView animated:YES completion:nil];
    }else if([object isKindOfClass:[Return class]]){
        Return * obj = _array[indexPath.row];
        UIAlertController * alertView = [UIAlertController alertControllerWithTitle:@"归还信息" message:nil preferredStyle:UIAlertControllerStyleAlert];
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"归还时间:%@",obj.retu_date];
        }];
        
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"管理员:%@",obj.manager_id];
        }];
        UIAlertAction * returnBook = [UIAlertAction actionWithTitle:@"删除" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            [_moc deleteObject:obj];
            [_moc save:nil];
            [_moc refreshAllObjects];
            [self goInit];
        ...
        Punish * obj = _array[indexPath.row];
        UIAlertController * alertView = [UIAlertController alertControllerWithTitle:@"处罚信息" message:nil preferredStyle:UIAlertControllerStyleAlert];
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"处罚时间:%@",obj.punish_date];
        }];
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"处罚金额:%d元",(int)obj.punish_free];
        }];
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"处罚原因:%@",obj.punish_tips];
        }];
        [alertView addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            textField.text = [NSString stringWithFormat:@"管理员:%@",obj.manager_id];
        }];
